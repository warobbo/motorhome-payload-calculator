 ...
 * - LT 15–18″ Continental / General: Databook TRA-standard kg-per-axle
 *   steps. LT 265/65 R17 uses the 2025 row where it differs at mid
 *   pressures. Lowest published bar step that covers the axle load.
 *   Do not divide the axle load by 2.
 * - LT / C 15–18″ Michelin Agilis CrossClimate: Michelin April 2025
 *   Inflation Charts (Light Truck, kg per axle). Brand Michelin / Agilis
 *   uses that book — not the Conti row for the same size.
 * - Rim not 15–18 (GREEN Conti/Michelin) or not 15–20 (AMBER LT) → refuse.
 * - C on 15–18″: Conti Van (ETRTO C) when that size + LI is embedded and
 *   the brand is not Michelin; otherwise Michelin Agilis C if listed;
 *   otherwise the ETRTO 3.75 / 4.50 bar interpolation (Conti / unmarked C).
 * - CP on 15–18″: Continental / General camping / VanContact Camper
 *   FA S / RA S columns (kg per axle). Not the plain C table. Not an
 *   Agilis C table. Michelin publishes no size-by-size CrossClimate
 *   Camping CP grid — never invent one, and never copy a Conti camping
 *   row and call it Michelin. Named Michelin / CrossClimate Camping /
 *   Agilis Camping CP is refused until Michelin’s official table is in
 *   the book. For Conti/General CP rear single (2 tyres), headline =
 *   max(tableBar, 5.5). Front CP stays on the table. Rear dual (RA T)
 *   does not use that floor.
 * - Same size, different load index → different row. Never reuse another LI.
 * - P-metric: no inflation table is invented.
 * - AMBER LT max-only (General Grabber AT2 / A/TX, BFGoodrich KO2):
 *   published max load @ max cold pressure. Not a full curve. Dual
 *   stays blank when the maker did not print one. Never invent a
 *   step from the load index.
 * - Goodyear / Yokohama: refused until those tables are added.
 * Fitter + the tyre maker’s own chart still win. Vehicle axle, wheel
 * and legal limits still win.
 */
"use strict";

(function (root) {
  var TraDb = root.TyreTraDb;
  var MichelinDb = root.TyreMichelinDb;
  var AmberDb = root.TyreAmberDb;
  try {
    if (typeof require === "function") {
      TraDb = require("./tyre-tra-db");
      try {
        MichelinDb = require("./tyre-michelin-db");
      } catch (micErr) { /* optional in older checkouts */ }
      try {
        AmberDb = require("./tyre-amber-db");
      } catch (ambErr) { /* optional in older checkouts */ }
    }
  ...
  /**
   * ETRTO camping guidance for CP tyres in single fitment on the rear
   * axle of a camper / motorhome: inflate to at least 5.5 bar (≈79–80 PSI)
   * even when the FA/RA load table would allow a lower step. Michelin UK
   * and Continental both cite this. Do not invent a front floor.
   */
  var ETRTO_CP_SINGLE_REAR_MIN_BAR = 5.5;

  var MICHELIN_CP_REFUSE =
    "We don’t have Michelin’s Camping CP load/pressure table for this size. We won’t invent one or copy another brand’s camping table and call it Michelin. Michelin UK has been asked for the official tables.";
  ...
  function speedRatingInfo(letter) {
    ...
    if (code === "ZR") {
      return { code: "ZR", kmh: 240, mph: 149, note: "over about 149 mph (240 km/h)" };
    }
  ...
  function serviceLabel(service, family) {
    if (service === "CP" || family === "CP") {
      return "CP — camping / motorhome tyre (not a plain van C tyre)";
    }
    if (service === "C" || family === "C") {
      return "C — reinforced van tyre";
    }
    if (service === "LT" || family === "LT") {
      return "LT — light truck tyre";
    }
    return "No C, CP or LT mark — often a car tyre";
  }
  ...
  function describeSidewall(parsed) {
    ...
    if (parsed.speed) {
      if (parsed.speed.note) {
        speed = "Speed rating " + parsed.speed.code + " — " + parsed.speed.note + ".";
      } else {
        speed = "Speed rating " + parsed.speed.code + " — up to about " + parsed.speed.mph +
          " mph (" + parsed.speed.kmh + " km/h)";
      }
    }
    ...
    return {
      ...
      extraLoad: parsed.extraLoad ? "XL / reinforced — a higher-load version of this size." : null,
      load: load,
      speed: speed
    };
  }
  ...
  /**
   * Read Michelin / Continental / later-brand from the brand box or sidewall.
   * Empty means “not named” — Conti first, then Michelin C/LT if Conti has no row.
   */
  function detectMaker(brandOrText, sidewallRaw) {
    var text = String(brandOrText || "") + " " + String(sidewallRaw || "");
    text = text.toLowerCase();
    if (/\b(goodyear|wrangler)\b/.test(text)) return "goodyear";
    if (/\b(bf\s*goodrich|\bbfg\b|all-terrain\s+t\/?a|\bko2\b)\b/.test(text)) return "bfgoodrich";
    if (/\b(yokohama|\byoko\b)\b/.test(text)) return "later";
    if (/\b(michelin|agilis|crossclimate)\b/.test(text)) return "michelin";
    if (/\b(continental|\bconti\b|vancontact)\b/.test(text)) return "continental";
    if (/\b(general|grabber)\b/.test(text)) return "general";
    return null;
  }

  function matchTraEntry(opts) {
    return matchPublishedEntry(opts);
  }

  function matchPublishedEntry(opts) {
    opts = opts || {};
    var maker = opts.maker || detectMaker(opts.brand, opts.sidewall);
    if (maker === "goodyear" || maker === "later") {
      return {
        ok: false,
        error: "brand-later",
        maker: maker,
    ...
    if (maker === "michelin") {
      if (opts.family === "CP") {
        return {
          ok: false,
          error: "michelin-cp-no-table",
          family: "CP",
          sizeKey: opts.sizeKey || null,
          message: MICHELIN_CP_REFUSE
        };
      }
      if (!MichelinDb) return { ok: false, error: "unknown-table" };
      return MichelinDb.matchEntry(opts);
    }
    var conti = TraDb ? TraDb.matchEntry(opts) : { ok: false, error: "unknown-table" };
    if (maker === "continental") return conti;
    if (maker === "general") {
      if (conti.ok) return conti;
      if (conti.error === "no-matching-li" || conti.error === "ambiguous") return conti;
      if (AmberDb) {
        var genNamed = AmberDb.matchEntry({
          sizeKey: opts.sizeKey,
          family: opts.family || "LT",
          loadIndex: opts.loadIndex,
          dualLoadIndex: opts.dualLoadIndex,
        ...
        if (genNamed.ok) return genNamed;
        if (genNamed.error === "no-matching-li" || genNamed.error === "ambiguous") return genNamed;
      }
      return conti;
    }
    if (conti.ok) return conti;
    if (MichelinDb && opts.family !== "CP" && (opts.family === "C" || opts.family === "LT")) {
      var mic = MichelinDb.matchEntry(opts);
      if (mic.ok) return mic;
      if (conti.error === "no-matching-li" || conti.error === "ambiguous") return conti;
      if (mic.error === "no-matching-li" || mic.error === "ambiguous") return mic;
    }
    if (conti.error === "no-matching-li" || conti.error === "ambiguous") return conti;
    if (AmberDb && (opts.family === "LT" || opts.family === "P" || !opts.family)) {
      var genOpen = AmberDb.matchEntry({
        sizeKey: opts.sizeKey,
        family: opts.family || "LT",
        loadIndex: opts.loadIndex,
        dualLoadIndex: opts.dualLoadIndex,
  ...
  function suggestChartId(parsed) {
    ...
    if (path.path === "lt-amber-max") return "lt-amber-max";
   ...
   * Do not divide by 2. tableBar = lowest published bar step that covers.
   * Headline bar = tableBar, except CP rear single which is
   * max(tableBar, 5.5) per ETRTO / Michelin UK camping guidance.
   */
  function ltColdBarForAxleLoad(opts) {
    ...
    return applyCpCampingGuidance(result);
  }
  ...
  function resolvePressurePath(parsed, extras) {
    ...
    var maker = extras.maker || detectMaker(extras.brand, parsed.raw);
    ...
    if (match.ok && match.entry.confidence === "AMBER") {
      if (AmberDb && !AmberDb.isSupportedRim(parsed.rimIn)) {
        return {
    ...
    if (match.ok && match.entry.family === "CP") {
      var cpMaker = match.maker || match.entry.maker || "continental";
      return {
        path: "cp-databook",
        family: "CP",
        confidence: "GREEN",
        maker: cpMaker,
        cpLoadSource: match.cpLoadSource || "continental",
        cpFloorSource: match.cpFloorSource || "etrto",
        tableId: match.entry.id,
        table: match.entry,
        label: match.entry.label,
        source: match.entry.source
    ...
    if (match.error === "michelin-cp-no-table") {
      return {
        path: "no-table",
        family: "CP",
        reason: "michelin-cp-no-table",
        maker: "michelin",
        sizeKey: parsed.sizeKey
      };
    }
    ...
    if (parsed.family === "C" && maker !== "michelin" && maker !== "bfgoodrich") {
      return {
        path: "c-etrto",
        family: "C",
        chartId: "c375",
        label: "van C tyre table",
  ...
  function formatPublishedBar(bar) {
    var n = toFiniteNumber(bar);
    if (n == null) return "";
  ...
  function applyCpCampingGuidance(lookup) {
    if (!lookup || !lookup.ok || lookup.status !== "ok") return lookup;
    if (!lookup.table || lookup.table.family !== "CP") return lookup;

    lookup.tableBar = lookup.bar;
    lookup.recommendedBar = lookup.bar;

    if (lookup.column === "rear") {
      var tableBar = lookup.bar;
      var recommendedBar = Math.max(tableBar, ETRTO_CP_SINGLE_REAR_MIN_BAR);
      lookup.tableBar = tableBar;
      lookup.recommendedBar = recommendedBar;
      lookup.appliedCpRearFloor = recommendedBar > tableBar + 1e-9;
      lookup.bar = recommendedBar;
      lookup.psi = roundPsi(barToPsi(recommendedBar));
      if (lookup.appliedCpRearFloor) {
        lookup.note = "Table for your axle weight: " + formatPublishedBar(tableBar) +
          " bar. Minimum for a camping tyre on the rear (two tyres): 5.5 bar. We show the higher.";
      }
      return lookup;
    }

    if (lookup.column === "dual") {
      lookup.appliedCpRearFloor = false;
      return lookup;
    }
  ...
  function coldPressureForAxle(opts) {
    ...
    if (pathInfo && pathInfo.path === "lt-amber-max") {
      var amber = pathInfo.table;
      var amberDual = tyresOnAxle >= 4;
      var amberEach = amberDual ? amber.dualKgEach : amber.singleKgEach;
      if (amberDual && amberEach == null) {
        return {
          ok: false,
          error: "no-table",
          reason: "amber-no-dual",
          family: "LT",
          maker: amber.maker,
          sizeKey: parsed.sizeKey,
          table: amber,
      ...
      var amberCap = amberEach * tyresOnAxle;
      var amberOut = {
        ok: true,
        status: axleLoadKg > amberCap + 1e-9 ? "over-capacity" : "amber-max-only",
        path: "lt-amber-max",
        confidence: "AMBER",
        bar: axleLoadKg > amberCap + 1e-9 ? null : amber.maxBar,
        psi: axleLoadKg > amberCap + 1e-9 ? null : amber.maxPsi,
        tableBar: amber.maxBar,
        recommendedBar: amber.maxBar,
        maxBar: amber.maxBar,
        maxKgEach: amberEach,
        maxKg: amberCap,
        capacityKg: amberCap,
        column: amberDual ? "dual" : "single",
        table: amber,
        axleLoadKg: axleLoadKg,
        loadKg: axleLoadKg
      };
      return attachAxleMeta(amberOut, {
        useDual: amberDual,
        usedIndex: usedIndex,
        tyresOnAxle: tyresOnAxle,
        axleLoadKg: axleLoadKg,
    ...
    if (pathInfo && isTablePath(pathInfo.path)) {
      ...
      return attachAxleMeta(lt, {
        ...
        axleLoadKg: axleLoadKg,
        loadKg: axleLoadKg,
        confidence: pathInfo.confidence || "GREEN",
        pathInfo: pathInfo,
        dualFallback: tyresOnAxle >= 4 && dualLoadIndex == null
      });
    }
  ...
  /**
   * True only when the calculator refused because the size, load index
   * or brand is not in the curated tables. Overload fails and incomplete
   * inputs are not capture targets. A result that already has bar/PSI
   * is never treated as uncovered.
   */
  function isUncoveredRefuse(result) {
    if (!result || result.ok) return false;
    if (result.bar != null || result.psi != null) return false;
    var err = result.error;
    return err === "no-table" ||
      err === "no-matching-li" ||
      err === "unknown-load-index" ||
      err === "unsupported-rim";
  }
  ...
  var api = {
    ...
    MICHELIN_CP_REFUSE: MICHELIN_CP_REFUSE,
    ...
    TraDb: TraDb,
    MichelinDb: MichelinDb,
    AmberDb: AmberDb,
    WAYNE_EXAMPLE: WAYNE_EXAMPLE,
    detectMaker: detectMaker,
    matchPublishedEntry: matchPublishedEntry,
    barToPsi: barToPsi,
    psiToBar: psiToBar,
    roundBar: roundBar,
    roundPsi: roundPsi,
    ...
    isUncoveredRefuse: isUncoveredRefuse
  };
