 ...
 * - CP on 15–18″: Continental camping / VanContact Camper FA S / RA S
 *   columns (kg per axle). Not the plain C table. Not an Agilis C table.
 *   Michelin publishes no size-by-size CrossClimate Camping CP grid —
 *   never invent one. Named Michelin / CrossClimate Camping CP uses the
 *   Conti camping row only when size + load index truly match, plus the
 *   ETRTO / Michelin UK 5.5 bar rear floor. Size + LI we do not have →
 *   refuse. For CP rear single (2 tyres), headline = max(tableBar, 5.5).
 *   Front CP stays on the table. Rear dual (RA T) does not use that floor.
 * - Same size, different load index → different row. Never reuse another LI.
 * - P-metric: no inflation table is invented.
 * - Goodyear / BFGoodrich / Yokohama: refused until those tables are added.
...
(function (root) {
  ...
  var MICHELIN_CP_SOURCE =
    "Load steps: Continental Tyre Databook VanContact Camper / CP for this size and load index. Michelin does not publish a size-by-size CrossClimate Camping CP grid — we will not invent one or use an Agilis C table. Rear single cold pressure is the higher of that Conti step and 5.5 bar (ETRTO camping / Michelin UK). Front stays on the Conti table.";
  ...
  function serviceLabel(service, family) {
    if (service === "CP" || family === "CP") {
      return "CP (camping / Camping Pneu) — a motorhome tyre with ETRTO special camping load capacities. Not the same as a plain C van tyre. Driving cold pressure uses the Continental VanContact Camper front-axle (FA S) and rear-axle (RA S) columns when that 15–18″ size and load index are in the book. Michelin CrossClimate Camping has no size-by-size CP grid — we use the Conti camping row only when size and load index match, never an Agilis C table. On a single rear (2 tyres) we never show less than 5.5 bar — ETRTO camping / Michelin UK / Continental owner advice — even if the table step is lower. Front stays on the table. Rear dual uses RA T only, with no 5.5 floor. Parked / site load can allow a temporary higher load at a higher pressure — the maker’s camping table and a fitter still win.";
    }
    if (service === "C" || family === "C") {
      return "C-rated commercial tyre — European van marking. Cold pressure uses the Continental Van (ETRTO C) steps, or the Michelin Agilis CrossClimate C table when you name Michelin / Agilis, for listed 15–18″ sizes. Otherwise the ETRTO C-type chart.";
    }
    if (service === "LT" || family === "LT") {
      return "LT-metric (light truck). Cold pressure comes from the Continental Tyre Databook TRA-standard table (kg per axle at bar) for 15–18″ sizes, or the Michelin Agilis CrossClimate LT table when you name Michelin / Agilis. The 2025 row is used for LT265/65R17 where it differs from older books. Not the old US lb/PSI extract and not the European C-type 3.75/4.50 bar formula.";
    }
  ...
  function detectMaker(brandOrText, sidewallRaw) {
    ...
    if (/\b(bf\s*goodrich|\bbfg\b|yokohama|\byoko\b)/.test(text)) return "later";
    if (/\b(michelin|agilis|crossclimate)\b/.test(text)) return "michelin";
    if (/\b(continental|\bconti\b|general|grabber|vancontact)\b/.test(text)) return "continental";
    return null;
  }
  ...
  function matchPublishedEntry(opts) {
    ...
    if (maker === "michelin") {
      if (opts.family === "CP") {
        var contiCp = TraDb ? TraDb.matchEntry(opts) : { ok: false, error: "unknown-table" };
        if (contiCp.ok && contiCp.entry && contiCp.entry.family === "CP") {
          return {
            ok: true,
            entry: contiCp.entry,
            maker: "michelin",
            cpLoadSource: "continental",
            cpFloorSource: "michelin-etrto"
          };
        }
        if (contiCp.error === "no-matching-li" || contiCp.error === "ambiguous") {
          return contiCp;
        }
        return {
          ok: false,
  ...
  function resolvePressurePath(parsed, extras) {
    ...
    if (match.ok && match.entry.family === "CP") {
      ...
      return {
        ...
        table: match.entry,
        label: match.entry.label,
        source: cpMaker === "michelin" ? MICHELIN_CP_SOURCE : match.entry.source
      };
    }
    ...
    if (parsed.family === "C" && maker !== "michelin") {
      return {
        path: "c-etrto",
        family: "C",
        chartId: "c375",
  ...
  function applyCpCampingGuidance(lookup) {
    ...
    if (lookup.column === "rear") {
      ...
      lookup.note = "Databook table for your axle load: " + formatPublishedBar(tableBar) +
        " bar. ETRTO CP single-rear camping minimum: 5.5 bar (Michelin UK / Continental owner advice). We show the higher.";
      return lookup;
    }

    if (lookup.column === "dual") {
      lookup.appliedCpRearFloor = false;
      lookup.note = "Rear dual (4 tyres): RA T table only. The 5.5 bar ETRTO CP single-rear camping minimum does not apply.";
      return lookup;
    }
  ...
  var api = {
    ...
    MICHELIN_CP_SOURCE: MICHELIN_CP_SOURCE,
