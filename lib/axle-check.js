...
(function (root) {
  ...
  function incompleteCopy(kind) {
    ...
    if (kind === "weights-only") {
      return {
        label: "Axle check incomplete",
        detail: "Enter plate axle limits to check"
      };
    }
    return {
      label: cautionLabel(),
  ...
  function tyresLinkCopy(hasWeights) {
    return hasWeights
      ? "Use these weights in the Tyres tool (weighbridge figures, not the plate ratings)."
      : "Fitted different tyres? Use the Tyres tool with your axle weights (weighbridge figures, not the plate ratings).";
  }
  ...
  function evaluate(input) {
    ...
    if (fail && rearOver) {
      ...
      detail = mode === MODE_LOADED
        ? "Rear over the plate even if total MAM is OK. Do not drive until you lose rear weight."
        : "Rear axle is over the plate on this empty ticket. That is a fail — use Loaded for a roadside check.";
    } else if (fail) {
      label = "Front axle over — fail";
      detail = mode === MODE_LOADED
        ? "Front axle is over the plate. That is a fail even if total MAM is OK."
        : "Front axle is over the plate on this empty ticket. That is a fail — use Loaded for a roadside check.";
    } else {
