...
function platformModels() {
  return Array.from(new Set(CATALOGUE.map(function (row) { return row.model; })));
}

/** Map MOT/DVLA casing (DUCATO, "Ducato 35") onto catalogue labels when we can. */
function canonicalModel(model) {
  const raw = String(model || "").trim();
  const key = norm(raw);
  if (!key) return "";
  const exact = CATALOGUE.find(function (row) { return norm(row.model) === key; });
  if (exact) return exact.model;
  const prefix = CATALOGUE.find(function (row) {
    return key.startsWith(norm(row.model));
  });
  return prefix ? prefix.model : raw;
}

function isCataloguePlatform(model) {
  const key = norm(model);
  if (!key) return false;
  return platformModels().some(function (name) {
    const p = norm(name);
