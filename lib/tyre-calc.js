 ...
 * - C / CP on 15–18″: prefer the book’s Van (ETRTO C) steps when that size
...
(function (root) {
  ...
  function serviceLabel(service, family) {
    if (service === "CP" || family === "CP") {
      return "CP (camping) — a motorhome / camper tyre, including higher load when the van is parked. Driving cold pressure uses the Continental van table when that size is in the book, otherwise the ETRTO C-type path.";
    }
  ...
  function isTablePath(path) {
    return path === "lt-databook" || path === "c-databook";
  }
  ...
  function ltColdBarForAxleLoad(opts) {
    ...
    var path = table.family === "C" ? "c-databook" : "lt-databook";
    var rows = table.rows;
  ...
  function resolvePressurePath(parsed) {
    ...
    var match = matchTraEntry({
      ...
      dualLoadIndex: parsed.dualLoadIndex,
      loadRange: parsed.loadRange
    });
