 ...
 *   uses that book — not the Conti row for the same size. No Michelin
 *   Camping CP grid: Michelin-branded CP is refused.
...
(function (root) {
  ...
  function detectMaker(brandOrText, sidewallRaw) {
    ...
    if (/\b(michelin|agilis)\b/.test(text)) return "michelin";
  ...
  function resolvePressurePath(parsed, extras) {
    ...
    if (maker === "michelin" && parsed.family === "CP") {
      return {
        path: "no-table",
        family: "CP",
    ...
    if (match.ok && match.entry.family === "CP") {
      return {
        path: "cp-databook",
        family: "CP",
        maker: match.entry.maker || "continental",
        tableId: match.entry.id,
        table: match.entry,
        label: match.entry.label,
        source: match.entry.source
