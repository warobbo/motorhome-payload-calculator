 ...
 *   columns (kg per axle). Not the plain C table. Size + LI we do not
 *   have → refuse. For CP rear single fitment (2 tyres), ETRTO camping
 *   guidance cited by Continental to UK motorhome owners is a 5.5 bar
 *   floor even when the load table would allow less (likely rear
 *   overload / unequal load). Headline = max(tableBar, 5.5). Front CP
 *   stays on the table. Rear dual (RA T) does not use that 5.5 floor.
...
(function (root) {
   ...
   * even when the FA/RA load table would allow a lower step. Continental
   * cites this to UK motorhome owners; VanContact Camper fact sheets also
   * give CP rear a special treatment vs plain C. Do not invent a front floor.
   */
   ...
   * max(tableBar, 5.5) per ETRTO camping guidance.
   */
  ...
  function applyCpCampingGuidance(lookup) {
    ...
    if (lookup.column === "rear") {
      ...
      lookup.note = "Databook table for your axle load: " + formatPublishedBar(tableBar) +
        " bar. ETRTO CP single-rear camping minimum: 5.5 bar. We show the higher.";
      return lookup;
    }
